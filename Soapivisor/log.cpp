...
LogInitialization(ULONG flag, const wchar_t *log_file_path) {
  ...
  // Test the log.
  status = Soapivisor_LOG_INFO("Log has been %sinitialized.",
                                  (need_reinitialization ? "partially " : ""));
  if (!NT_SUCCESS(status)) {
    goto Fail;
  }
  Soapivisor_LOG_DEBUG("Info= %p, Buffer= %p %p, File= %S",
...
_Use_decl_annotations_ static NTSTATUS LogpInitializeBufferInfo(
  ...
  if (status == STATUS_OBJECT_PATH_NOT_FOUND) {
    Soapivisor_LOG_INFO("The log file needs to be activated later.");
...
_Use_decl_annotations_ void LogRegisterReinitialization(
  ...
  Soapivisor_LOG_INFO("The log file will be activated later.");
}
...
_Use_decl_annotations_ VOID static LogpReinitializationRoutine(
  ...
  if (NT_SUCCESS(status)) {
    Soapivisor_LOG_INFO("The log file has been activated.");
  }
}
...
_Use_decl_annotations_ void LogIrpShutdownHandler() {
  ...
  Soapivisor_LOG_DEBUG("Flushing... (Max log usage = %Iu/%lu bytes)",
                          g_logp_log_buffer_info.log_max_usage,
                          kLogpBufferSize);
  Soapivisor_LOG_INFO("Bye!");
...
_Use_decl_annotations_ void LogTermination() {
  ...
  Soapivisor_LOG_DEBUG("Finalizing... (Max log usage = %Iu/%lu bytes)",
                          g_logp_log_buffer_info.log_max_usage,
                          kLogpBufferSize);
  Soapivisor_LOG_INFO("Bye!");
  g_logp_debug_flag = kLogPutLevelDisable;
...
_Use_decl_annotations_ static VOID LogpBufferFlushThreadRoutine(
  ...
  Soapivisor_LOG_DEBUG("Log thread started (TID= %p).",
