...
_Use_decl_annotations_ EptData *EptInitialization() {
  ...
  const auto ept_data = static_cast<EptData *>(ExAllocatePoolZero(
      NonPagedPool, sizeof(EptData), kSoapivisorCommonPoolTag));
  if (!ept_data) {
    return nullptr;
  }
...
EptpAllocateEptEntryFromPreAllocated(EptData *ept_data) {
  ...
  if (count > kEptpNumberOfPreallocatedEntries) {
    Soapivisor_COMMON_BUG_CHECK(
        SoapivisorBugCheck::kExhaustedPreallocatedEntries, count,
...
#pragma warning(disable : 6001)
    ExFreePoolWithTag(preallocated_entries[i], kSoapivisorCommonPoolTag);
#pragma warning(pop)
  }
  ExFreePoolWithTag(preallocated_entries, kSoapivisorCommonPoolTag);
}
...
_Use_decl_annotations_ static void EptpDestructTables(EptCommonEntry *table,
    ...
    if (entry.fields.physial_address) {
      ...
      switch (table_level) {
        ...
        case 2:  // table == PDT, sub_table == PT
          ExFreePoolWithTag(sub_table, kSoapivisorCommonPoolTag);
          break;
        default:
          Soapivisor_COMMON_DBG_BREAK();
          break;
  ...
  ExFreePoolWithTag(table, kSoapivisorCommonPoolTag);
}
