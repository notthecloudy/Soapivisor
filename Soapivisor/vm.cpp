...
_IRQL_requires_max_(PASSIVE_LEVEL) static bool VmpIsSoapivisorInstalled();
...
#pragma alloc_text(PAGE, VmpIsSoapivisorInstalled)
...
_Use_decl_annotations_ static bool VmpIsVmxAvailable() {
  ...
  if (!cpu_features.fields.vmx) {
    Soapivisor_LOG_ERROR("VMX features are not supported.");
    return false;
  }
  ...
  if (static_cast<memory_type>(vmx_basic_msr.fields.memory_type) !=
    ...
    Soapivisor_LOG_ERROR("Write-back cache type is not supported.");
    return false;
  }
  ...
  if (!vmx_feature_control.fields.lock) {
    Soapivisor_LOG_INFO("The lock bit is clear. Attempting to set 1.");
  ...
  if (!vmx_feature_control.fields.enable_vmxon) {
    Soapivisor_LOG_ERROR("VMX features are not enabled.");
    return false;
  }

  if (!EptIsEptAvailable()) {
    Soapivisor_LOG_ERROR("EPT features are not fully supported.");
    return false;
  }
  return true;
}
...
_Use_decl_annotations_ static NTSTATUS VmpSetLockBitCallback(void *context) {
  ...
  if (!vmx_feature_control.fields.lock) {
    Soapivisor_LOG_ERROR("The lock bit is still clear.");
...
_Use_decl_annotations_ static SharedProcessorData *VmpInitializeSharedData() {
  ...
  Soapivisor_LOG_DEBUG("shared_data           = %p", shared_data);

  // Setup MSR bitmap
  shared_data->msr_bitmap = VmpBuildMsrBitmap();
  if (!shared_data->msr_bitmap) {
    ExFreePoolWithTag(shared_data, kSoapivisorCommonPoolTag);
    return nullptr;
  }
  ...
  if (!io_bitmaps) {
    ExFreePoolWithTag(shared_data->msr_bitmap, kSoapivisorCommonPoolTag);
    ExFreePoolWithTag(shared_data, kSoapivisorCommonPoolTag);
    return nullptr;
  }
...
_Use_decl_annotations_ static void *VmpBuildMsrBitmap() {
  ...
  const auto msr_bitmap =
      ExAllocatePoolZero(NonPagedPool, PAGE_SIZE, kSoapivisorCommonPoolTag);
  if (!msr_bitmap) {
    return nullptr;
  }
...
_Use_decl_annotations_ static UCHAR *VmpBuildIoBitmaps() {
  ...
  const auto io_bitmaps = static_cast<UCHAR *>(ExAllocatePoolZero(
      NonPagedPool, PAGE_SIZE * 2, kSoapivisorCommonPoolTag));
  if (!io_bitmaps) {
    return nullptr;
  }
...
_Use_decl_annotations_ static NTSTATUS VmpStartVm(void *context) {
  ...
  Soapivisor_LOG_INFO("Initialized successfully.");
  return STATUS_SUCCESS;
}
...
_Use_decl_annotations_ static void VmpInitializeVm(
  ...
  const auto processor_data = static_cast<ProcessorData *>(ExAllocatePoolZero(
      NonPagedPool, sizeof(ProcessorData), kSoapivisorCommonPoolTag));
  if (!processor_data) {
    return;
  }
...
_Use_decl_annotations_ static bool VmpEnterVmxMode(
  ...
  Soapivisor_LOG_DEBUG("IA32_VMX_CR0_FIXED0   = %08Ix", cr0_fixed0.all);
  Soapivisor_LOG_DEBUG("IA32_VMX_CR0_FIXED1   = %08Ix", cr0_fixed1.all);
  Soapivisor_LOG_DEBUG("Original CR0          = %08Ix", cr0_original.all);
  Soapivisor_LOG_DEBUG("Fixed CR0             = %08Ix", cr0.all);
  ...
  Soapivisor_LOG_DEBUG("IA32_VMX_CR4_FIXED0   = %08Ix", cr4_fixed0.all);
  Soapivisor_LOG_DEBUG("IA32_VMX_CR4_FIXED1   = %08Ix", cr4_fixed1.all);
  Soapivisor_LOG_DEBUG("Original CR4          = %08Ix", cr4_original.all);
  Soapivisor_LOG_DEBUG("Fixed CR4             = %08Ix", cr4.all);
...
_Use_decl_annotations_ static void VmpLaunchVm() {
  ...
  if (error_code) {
    Soapivisor_LOG_WARN("VM_INSTRUCTION_ERROR = %Iu", error_code);
  }
  ...
  if (vmx_status == VmxStatus::kErrorWithStatus) {
    ...
    Soapivisor_LOG_ERROR("VM_INSTRUCTION_ERROR = %Iu", error_code);
  }
...
_Use_decl_annotations_ void VmTermination() {
  ...
  Soapivisor_LOG_INFO("Uninstalling VMM.");
  auto status = UtilForEachProcessor(VmpStopVm, nullptr);
  if (NT_SUCCESS(status)) {
    Soapivisor_LOG_INFO("The VMM has been uninstalled.");
  } else {
    Soapivisor_LOG_WARN("The VMM has not been uninstalled (%08x).", status);
  }
  NT_ASSERT(!VmpIsSoapivisorInstalled());
}
...
_Use_decl_annotations_ static void VmpFreeProcessorData(
  ...
  ExFreePoolWithTag(processor_data, kSoapivisorCommonPoolTag);
}
...
_Use_decl_annotations_ static void VmpFreeSharedData(
  ...
  Soapivisor_LOG_DEBUG("Freeing shared data...");
  ...
  ExFreePoolWithTag(processor_data->shared_data, kSoapivisorCommonPoolTag);
}
